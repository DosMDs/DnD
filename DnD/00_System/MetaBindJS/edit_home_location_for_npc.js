...
try {
  ...
  const selected = await engine.prompt.suggester({
    placeholder: "Выбери домашнюю локацию для NPC",
    options: [clearOption, ...fileOptions],
  });

  if (!selected) {
    new Notice("Изменение домашней локации отменено.");
    return;
  }
  ...
  await helpers.updateFrontmatter(app, currentFile, (fm) => {
    if (selected === "__CLEAR__") {
      ...
      fm.home_location = helpers.buildDisplayLinkFromFile(app, selected);
    }
    fm.updated = helpers.today();
  });

  if (selected === "__CLEAR__") {
    new Notice("Домашняя локация очищена.");
  } else {
    new Notice(
      `Домашняя локация обновлена: ${helpers.getFileTitle(app, selected, selected.basename)}`,
    );
  }
} catch (error) {
  new Notice(error.message || "Ошибка изменения домашней локации.");
  console.error(error);
}
