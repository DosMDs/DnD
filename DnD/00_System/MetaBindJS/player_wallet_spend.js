...
try {
  ...
  if (amount > prev) {
    new Notice(`Недостаточно монет: ${prev} < ${amount}. Трата отменена.`);
    return;
  }
  ...
  new Notice(`-${amount} ${label}. Осталось: ${prev - amount} ${label}`);
} catch (error) {
  new Notice(error.message || "Ошибка траты монет.");
  console.error(error);
}
