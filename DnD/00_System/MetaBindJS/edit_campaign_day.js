...
try {
  ...
  const dayDelta = helpers.normalizeInteger(
    frontmatter.world_day_delta ?? 0,
    "world_day_delta",
  );
  if (dayDelta === 0) {
    throw new Error("Укажи ненулевое значение в поле world_day_delta.");
  }

  const currentState =
    helpers.getCampaignWorldStateFromFrontmatter(frontmatter);
  const nextTotalHours = currentState.world_total_hours + dayDelta * 24;
  ...
  await helpers.updateFrontmatter(app, currentFile, (fm) => {
    ...
    fm.world_day_delta = 0;
    fm.updated = helpers.today();
  });

  new Notice(`Игровая дата обновлена: ${nextState.world_date_label}`);
} catch (error) {
  new Notice(error.message || "Ошибка изменения игрового дня.");
  console.error(error);
}
