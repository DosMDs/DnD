...
try {
  ...
  if (!currentList.length) {
    throw new Error("У лога нет связанных локаций для удаления.");
  }
  ...
  const selectedLink = await engine.prompt.suggester({
    placeholder: "Выбери локацию для удаления из лога",
    options,
  });
  ...
  new Notice(`Локация удалена из лога: ${removedTitle}`);
} catch (error) {
  new Notice(error.message || "Ошибка удаления локации из лога.");
  console.error(error);
}
