...
try {
  ...
  if (!currentList.length) {
    throw new Error("У предмета нет связанных квестов для удаления.");
  }
  ...
  const selectedLink = await engine.prompt.suggester({
    placeholder: "Выбери квест для удаления у предмета",
    options,
  });
  ...
  new Notice(`Квест удалён у предмета: ${removedTitle}`);
} catch (error) {
  new Notice(error.message || "Ошибка удаления квеста у предмета.");
  console.error(error);
}
